...
L_z     = 1500;
z_i = 636.626;
p_count = 25; %for Ug2 and Neutral Channel
ustar   = 0.50;
Ugal    = 0.0;
T_scale = 1;
% Ug      = 10;
...
cd '/home/ahsan/Dropbox/LEScode_ahsan_freeForm/output/'

SS ='';
SSs =''; %prefix for scalars empty means not present!
Sprint = '-depsc';
%Tend      = 55000; %for Ug10 timeseries
%Tend      = 107000; %for Ug2 timeseries
Tend = 995; % NeutralChannel
Tstart    = Tend*3/10; %Andren's case averages over last 3/10th
dtr       = 1.5; %physical time in seconds
...
width = 4.00; height = width/1.2; rendeng = 'painters';
%***********************************************************
%TIME SERIES PLOTS
...
#    ylabel('$zf/u_*$','FontSize',10, 'interpreter', 'latex');
...
#        P  = [0.641905 0.791389 0.203571 0.117738];
#        xlabel('Pr_{SGS}','FontSize',10, 'interpreter', 'latex');
#        ylabel('$zf/u_*$','FontSize',10, 'interpreter', 'latex');
...
#        P  = [0.241905 0.791389 0.203571 0.117738];
#        xlabel('$\beta_\theta$','FontSize',10, 'interpreter', 'latex');
#        ylabel('$zf/u_*$','FontSize',10, 'interpreter', 'latex');
#        set(gca,'FontSize',10,'XLim',[0 1.5]);
...
#%         Sp  = Sp/((h-0.5)/(Nz-1));
#%        Sp  = fx'*z_i.*Sp/(mu2(h));
#        Sp  = (fx'.*z_i).^(1).*Sp/(mu2(h));
#        loglog(f(1:Nx/(2*fgr)),Sp(1:Nx/(2*fgr))'); hold all; %Explicit Filtering
...
#%     set(gca, 'Position', [pos(1)*1.2,.15,pos(3),0.8])
#%     set(hfig, 'visible', 'on');
#% %     mov = avifile('adectingIrMovie.avi','fps',10);
#% 
#%     for kk = 0.1:0.1:8
#%     clf();
#%             for h = 4:3:2*Nz/4
...
#%                 f   = fx*mBLA;
#%                 Sp  = (fx'.*z_i).^(1).*Sp/(mu2(h));
#%                 plot(f(1:Nx/(2*fgr)),Sp(1:Nx/(2*fgr))'); hold all; %Explicit Filtering
#%             end
#% 
#%         set(gca,'FontSize', 10); hold on;
#% 
#%         % plot -2/3 slope line
#%         xx = logspace(-0.699,1,10); cc =log10(0.08)-(-2/3.0)*log10(0.2); yy =  xx.^(-2/3.0)*10^(cc);
#%         plot(xx,yy, '-k', 'markerfacecolor', 'None', 'markeredgecolor', 'r', 'markersize', 5, 'LineWidth',1); hold on;
#%         jj = 1;
#%         Q = kk/2;
#%         grey = [0.64,0.8,0.35];
#%         purple = [0.59, 1, 1];
#%         H = fill([Q/2, Q, Q, Q/2],[0.001,0.001,0.1, 0.1],grey);
#%         set(H,'FaceAlpha',0.1); 
#%         HH = plot([kk kk], [0.001 0.1], 'r', 'linewidth', 2);
#%         HHH = fill([kk-Q, kk+Q, kk+Q, kk-Q],[0.001,0.001,0.1, 0.1],purple);
#%         text(kk-Q+ 0.1*(kk-Q),0.005,'[K-Q, K+Q]')
#%         text(kk+0.1*kk,0.003,'K')
#%         text(Q/2+0.1*Q/2,0.004,'[Q/2 Q]')
#%         set(HHH,'FaceAlpha',0.1);
#%         xx=xlabel('$k_1\Delta$','FontSize',10,'interpreter','latex');
...
#%     end
#%     
#%     SPR = sprintf('%s%s',SS,'GoMRI_specU_premultiplied_proposal'); print(Sprint,SPR);
#% %     mov = close(mov);
#%     
#%     
#%     %% ir locality straining mode
#%     
#%     kk = 1 ;    
#%     S = loadbin('spectru.bin',Nz,'l');
#%     R = Nx/2+1;
...
#%         xlim([0.01,15]); ylim([0.001, 0.10]);
#%    
#%     SPR = sprintf('%s%s',SS,'ir-locality-demo'); print(Sprint,SPR);
#%     
#%     
#%         %% ir locality straining mode small scale stress large scale strain
#%         
#%     kk = 1;
#%          
#%     S = loadbin('spectru.bin',Nz,'l');
...
#%         set(H,'FaceAlpha',0.1); 
#%         HH = plot([kk kk], [0.001 0.1], 'r', 'linewidth', 2);
#%         text(kk+0.1*kk,0.003,'stress', 'interpreter','latex')
#%         text(Q/2+0.1*Q/2,0.004,'strain', 'interpreter','latex')
#%         xx=xlabel('$k_1\Delta$','FontSize',10,'interpreter','latex');
#%         set(gca,'XScale','log','YScale','log')
#%         ylabel('$k_1E_u(k_1)/\sigma_u^2$','FontSize',10, 'interpreter', 'latex');
#%         xlim([0.01,15]); ylim([0.001, 0.10]);
#%    
#%     SPR = sprintf('%s%s',SS,'small-scale-stress-large-scale-strain-1'); print('-djpeg',SPR);
#%     
#%     
#%     
#%     
...
#%         grey = [0.64,0.8,0.35];
#%         purple = [0.59, 1, 1];
#%         H = fill([P, 2*P, 2*P, P],[0.001,0.001,0.1, 0.1],grey);
#%         set(H,'FaceAlpha',0.1); 
#%         HH = plot([kk kk], [0.001 0.1], 'r', 'linewidth', 2);
#%         text(kk+0.1*kk,0.003,'k', 'interpreter','latex')
#%         text(2*P+0.1*2*P,0.004,'[P 2P]', 'interpreter','latex')
#%         xx=xlabel('$k_1\Delta$','FontSize',10,'interpreter','latex');
#%         set(gca,'XScale','log','YScale','log')
#%         ylabel('$k_1E_u(k_1)/\sigma_u^2$','FontSize',10, 'interpreter', 'latex');
#%         xlim([0.01,15]); ylim([0.001, 0.10]);
#%    
#%     SPR = sprintf('%s%s',SS,'small-scale-stress-large-scale-strain-uv'); print('-djpeg',SPR);
#%     %%
#%     
#%              
#%     S = loadbin('spectru.bin',Nz,'l');
...
#%         grey = [0.64,0.8,0.35];
#%         purple = [0.59, 1, 1];
#%         H = fill([P/2, P, P, P/2],[0.001,0.001,0.1, 0.1],grey);
#%         set(H,'FaceAlpha',0.1); 
#%         HH = plot([kk kk], [0.001 0.1], 'r', 'linewidth', 2);
#%         text(kk+0.1*kk,0.003,'Strain', 'interpreter','latex')
#%         text(P+0.1*P/2,0.004,'stress', 'interpreter','latex')
#%         xx=xlabel('$k_1\Delta$','FontSize',10,'interpreter','latex');
#%         set(gca,'XScale','log','YScale','log')
#%         ylabel('$k_1E_u(k_1)/\sigma_u^2$','FontSize',10, 'interpreter', 'latex');
#%         xlim([0.01,15]); ylim([0.001, 0.10]);
#%    
#%     SPR = sprintf('%s%s',SS,'small-scale-stress-large-scale-strain-2'); print('-djpeg',SPR);
#  %%  
#    % regular spectra
#    hfig = figure(); 
#    set(gcf, 'PaperUnits', 'inches', 'PaperPosition', [0 0 width height]);
...
#        loglog(f(1:Nx/(2*fgr)),Sp(1:Nx/(2*fgr))'); hold all; %Explicit Filtering
#    end
#    xx = logspace(.1,2,10); cc =log10(0.006)-(-5/3.0)*log10(6); yy =  xx.^(-5/3.0)*10^(cc);
#    
#    
#    loglog(xx,yy, '-g', 'markerfacecolor', 'None', 'markeredgecolor', 'r', 'markersize', 5, 'LineWidth',1); hold on;
#    xx=xlabel('$kz$','FontSize',12,'interpreter','latex');
#    ylabel('$E_{u}/z$','FontSize',12, 'interpreter', 'latex');
#    xlim([0.1, 1100]); ylim([0.00001, 0.1]); 
#    
#    SPR = sprintf('%s%s',SS,'GoMRI_specU'); print(Sprint,SPR);  
#    
#%%
#    %%%%%%%%%Spectra - v %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
#    ylabel('$k_1E_v(k_1)/\sigma_v^2$','FontSize',10, 'interpreter', 'latex');
...
#    ylabel('$k_1E_w(k_1)/\sigma_w^2$','FontSize',10, 'interpreter', 'latex');
#    xlim([0.3,40]); ylim([0.001, 0.10]);
...
#        hold off;
#        xlabel('$k_1z$','FontSize',10, 'interpreter', 'latex');
#        ylabel('$E_T(k)/z$','FontSize',10, 'interpreter', 'latex');
